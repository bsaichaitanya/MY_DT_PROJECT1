...
public class CartController {
	...
	public String addToCart(@PathVariable("pro_id") String pro_id, HttpSession session, Model model,RedirectAttributes redirectAttributes) {
			...
			if (cartList == null) {
				...
				for (int i = 0; i < cartList.size(); i++) {
					if (product.getPro_id() == cartDAO.getByUser(loggedInUser,"N").get(i).getProductID()) {
						cart = cartList.get(i);
						if(cart.getQuantity()<product.getPro_quantity()){
							...
							return "redirect:/Home";
							 ...
							 String proID =cartList.get(i).getProductID();
