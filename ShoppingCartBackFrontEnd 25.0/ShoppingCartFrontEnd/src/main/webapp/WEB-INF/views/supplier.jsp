			...
			<form:form action="${addAction}" commandName="supplier" class="form-responsive form-horizontal">
						<div class="col-md-12">
							...
							<div class="form-group">
								<form:label path="sup_id" class="control-label col-md-4">
									<spring:message text="ID" />
								</form:label>
								<div class="col-md-4">
									<c:choose>
										<c:when test="${!empty supplier.sup_id}">
											<form:input path="sup_id" readonly="true" class="form-control" />
										</c:when>
										<c:otherwise>
											<form:input path="sup_id" pattern="{5,10}" required="true" title="id should be between 5 to 10 characters" class="form-control" />
										</c:otherwise>
									</c:choose>
								</div>
							</div>
							...
							<div class="form-group">
								<form:label path="sup_name" class="control-label col-md-4"><spring:message text="NAME" /></form:label>
								<div class="col-md-4">
									<form:input path="sup_name" required="true" class="form-control" />
								</div>
							</div>
							
							
							<div class="form-group">
							<form:label path="sup_address"	class="control-label col-md-4"><spring:message text="ADDRESS" /></form:label>
								<div class="col-md-4">
									<form:input path="sup_address" required="true" class="form-control" />
								</div>
							</div>
	...
	<c:if test="${!empty supplierList}">
		...
		<c:forEach items="${supplierList}" var="supplier">
			<tr>
				...
				<td><a href="<c:url value="supplier/remove/${supplier.sup_id}"/>" onclick="myFunction()">Delete</a></td>
			</tr>
		</c:forEach>
		</tbody>
		</table>
